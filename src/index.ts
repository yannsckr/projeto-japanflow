      ...
      events: {
        ...
        items: {
          ...
          properties: {
            ...
            targetInfo: {
              type: Type.STRING,
              description: "Nome do setor, 'todos' ou nomes separados por vírgula.",
            },
          },
...
export const parseCalendarEvents = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      const functionCall = response.functionCalls?.find((call) => call.name === 'create_events');
      ...
      if (!functionCall) {
        ...
        throw new HttpsError('internal', 'A IA não retornou os eventos esperados.');
      }
      ...
      if (status === 402) {
        throw new HttpsError('failed-precondition', 'Créditos da API de IA insuficientes.');
      }

      throw new HttpsError('internal', 'Não foi possível processar os eventos do calendário.');
    }
  }
);
